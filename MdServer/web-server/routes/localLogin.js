...
var tokenService = require('../../shared/tokenService.js');
...
exports.login = function(req, res){
  ...
  if(!userName || !password) {
    ...
    res.send({code: CODE.LOGIN.LOGIN_ILLEGAL});
    return;
  }
  // 去玩家账号库里面查找此账号信息
  dbService.selectUserinfo(userName, function(userinfo) {
    ...
    if(!userinfo.length) {
      console.log('没有此用户名');
      res.send({code: CODE.LOGIN.NO_USER});
      return;
    }
    // 密码不正确
    if(crypto.md5(password) != userinfo[0].password) {
      console.log('用户名密码不匹配');
      res.send({code: CODE.LOGIN.ERR_PASSWORD});
      return;
    }
    // 验证成功，返回gate服务器地址
    console.log('登录成功');
...
exports.register = function(req, res) {
  ...
  readFile.checkIllegalWord(userName, function(result) {
    if(result) {
      ...
      res.send({code: CODE.LOGIN.REJECT});
      return;
    }
    // 先去账号库查找看这个账号是否已被注册
    dbService.selectUserinfo(userName, function(userinfo) {
      ...
      var registerType = CONST.REGISTER_TYPE.LOCAL;
      // 若账号未被注册，则写库注册
...
//~~~~~~~~~~~~~~~~~~~ 测试使用逻辑 ~~~~~~~~~~~~~~~~~~~~~~
  ...
  var registerType = CONST.REGISTER_TYPE.QUICK;       //注册类型：快速注册
